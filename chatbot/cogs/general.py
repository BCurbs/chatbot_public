...
class General(Cog):
    ...
    async def _help_all(self, ctx):
        ...
        info = discord.Embed(title='Chatbot: Info', description='A fun chatbot that learns as you talk to it. ',
                             color=discord.Color.blue())
        info.set_thumbnail(url=self.bot.user.avatar_url)
        info.add_field(name='About',
                       value="Chatbot: A fun chatbot written using the chatterbot library in python by @BCurbs#3937.")
        ...
        #                value="Chatbot is open source! Feel free to view and contribute to our Python code "
        #                      "[on Github](https://github.com/FRCDiscord/Dozer)", inline=False)
        info.add_field(name='Support',
                       value="If you have any questions or comments you can join our [discord server](https://discord.gg/bB8tcQ8) ")
        ...
        info.set_footer(text='Chatbot Help | all commands | Info page')
        await self._show_help(ctx, info, 'Chatbot: Commands', '', 'all commands', ctx.bot.commands)
    ...
    async def _help_command(self, ctx, command):
        ...
        info.set_footer(text='Chatbot Help | {!r} command | Info'.format(command.qualified_name))
    ...
    async def _show_help(self, ctx, start_page, title, description, footer, commands, **format_args):
        ...
        footer = 'Chatbot Help | {} | Page {}'.format(footer, '{page_num} of {len_pages}')
